...
// or a generic "GET /api/results/metrics" logged by the audit middleware
// for view/list/download traffic that doesn't have its own log_activity()
// call — see app/audit/middleware.py).
...
export default function ActivityLogPage() {
	...
	return (
		<div className="space-y-6 max-w-7xl mx-auto">
			...
			<div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-2 border-b border-gray-200">
				<div>
					...
					<p className="text-xs text-gray-500 mt-0.5 font-medium">
						Full audit trail of all actions — immutable. {total > 0 && `${total} total entries.`}
					</p>
				</div>
			...
			{/* FILTER CONSOLE — user + search + timeline */}
									...
									<div className="flex-1 p-4 flex flex-col justify-between space-y-2">
										<div className="flex flex-wrap items-center gap-2">
											...
											<div className="flex items-center gap-1 text-xs">
												...
												<span className="font-black text-[#1E3A5F]">{log.user_email || "System Pipeline"}</span>
											</div>
										</div>
										...
										<div className="flex flex-wrap items-center gap-2 text-[10px] font-mono text-gray-400 pt-0.5">
											<span className="font-bold uppercase tracking-wider text-[9px]">Action:</span>
											<span className="font-semibold text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded-xs">{log.action}</span>
											{log.ip_address && (
												<>
													<span className="font-bold uppercase tracking-wider text-[9px]">IP:</span>
													<span className="font-semibold text-gray-500 bg-gray-100 px-1.5 py-0.5 rounded-xs">{log.ip_address}</span>
												</>
											)}
										</div>
									</div>
