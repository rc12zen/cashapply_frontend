...
const navItems: {
	...
	{ href: "/bank-accounts", label: "Accounts & OU's", icon: Landmark, section: "Settings", requiresPermission: "canViewData" },
...
function AppShell({ children }: { children: React.ReactNode }) {
	...
	const handleSignOut = async () => {
		...
		document.cookie =
			`login_user_email_stub=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT${secureSuffix}`;
		if (!IS_LOCAL_DEV) {
			...
			<header className="h-20 bg-[#222222] text-white px-6 flex items-center justify-between sticky top-0 z-30 shadow-xs border-b border-white/10 shrink-0 relative">
				...
				<div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2">
					{/* eslint-disable-next-line @next/next/no-img-element -- plain <img>
					     ...
					     (for layout-shift prevention) that violates strict style-src
					     (no 'unsafe-inline') -- confirmed by testing. Same reasoning as
					     the Z-logo gif below already used a plain <img> for. */}
					<img
						src="/logo.png"
						alt="Zensar Logo"
						width={90}
