...
export default function RunControlBar({
  ...
  return (
    <div
      className={`px-6 py-5 flex flex-col sm:flex-row items-center justify-between gap-4 border shadow-sm transition-all duration-300 rounded-2xl
    ${
        isRunning || loading
          ? "bg-[#222222] border-black text-white shadow-lg" // Original dark loading state
          : filesAlreadyAnalyzed
            ? "bg-amber-50 border-amber-200 shadow-md text-gray-900" // Already analyzed — needs a fresh upload
            : agingStatus.loaded && files.length > 0
              ? "bg-[#222222] border-black shadow-xl text-white" // Strong emphasis when ready — on-brand black, not blue
              : "bg-white border-gray-200 shadow-sm text-gray-900" // White background when missing files/unready
      }`}
    >
                ...
                {runnableSelectedCount > 0 ? (
                  ...
                  <>
                    ...
                    {duplicateRunIds.length === 1 ? (
                      ...
                      <a href="/analysis-history" className="underline font-bold text-white hover:text-white/80">
                        view analysis history
                      </a>
                    )}
      ...
      ${
          !(isRunning || loading) && agingStatus.loaded && files.length > 0 && !filesAlreadyAnalyzed
            ? "bg-white text-[#222222] hover:bg-gray-100" // Inverse button style for the strong black background
            : "bg-[#222222] text-white hover:bg-[#3A3A3A]" // Standard button theme style
        }`}
      >
        {filesAlreadyAnalyzed ? (
          <>
