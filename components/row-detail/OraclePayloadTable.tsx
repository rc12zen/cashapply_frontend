 ...
 */
import { FxView, fmt } from "./types";
export function OraclePayloadTable({ payload, creditAmount, fx }: { payload: Record<string, any>; creditAmount: number; fx?: FxView }) {
  ...
  // ReferenceAmount is in INVOICE currency (stated_amount / outstanding_amount
  // — see oracle/fusion_client.py). So balance + share % must be computed
  // against the credited amount CONVERTED to invoice currency, not the raw
  // creditAmount (credited currency), which would false-flag cross-currency rows.
  const creditInInvoiceCcy = fx ? fx.credit_amount_invoice_ccy : creditAmount;
  const sumOk = refs.length === 0 || Math.abs(sumRefs - creditInInvoiceCcy) < 0.02;
                ...
                {refs.map((ref, i) => (
                  <tr key={i} className="hover:bg-blue-50/30">
                    ...
                    <td className="px-3 py-2.5 font-mono text-right text-gray-400 text-[10px]">
                      {creditInInvoiceCcy > 0 ? `${((Number(ref.ReferenceAmount) / creditInInvoiceCcy) * 100).toFixed(1)}%` : "—"}
                    </td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="border-t-2 border-gray-200 bg-gray-50">
                <tr>
                  ...
                  <td className="px-3 py-2 font-mono text-right text-gray-400 text-[10px]">
                    {creditInInvoiceCcy > 0 ? `${((sumRefs / creditInInvoiceCcy) * 100).toFixed(1)}%` : "—"}
                  </td>
                </tr>
              </tfoot>
            </table>
