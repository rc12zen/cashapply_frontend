        ...
        <div className="bg-white border border-gray-200 rounded-sm p-3 flex flex-col lg:flex-row lg:items-center gap-3">
            ...
            {TIME_PERIODS.map((p) => (
              <button
                ...
                className={`px-3 py-1 text-[10px] font-bold uppercase tracking-wider rounded-xs transition-all cursor-pointer whitespace-nowrap ${
                  timePeriod === p ? "bg-[#1E3A5F] text-white shadow-xs" : "text-gray-500 hover:text-primary"
                }`}
              >
                {p}
              </button>
          ...
          {timePeriod === "Custom Date" && (
            <div className="flex items-center gap-2">
              <input type="date" value={customStart} onChange={(e) => setCustomStart(e.target.value)}
                className="bg-gray-50 border border-gray-300 rounded-sm text-xs font-bold text-primary px-2 py-1.5 outline-none focus:border-[#4A90E2]" />
          ...
          <div className="flex items-center gap-2 lg:ml-auto">
            <select value={selectedBank} onChange={(e) => setSelectedBank(e.target.value)}
              className="bg-gray-50 border border-gray-300 text-xs font-bold text-primary px-3 py-1.5 rounded-sm outline-none focus:border-[#4A90E2] cursor-pointer">
