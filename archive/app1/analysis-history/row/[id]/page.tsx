  ...
  return (
    <div className="flex items-center justify-between gap-3 px-5 py-3 border-b border-gray-100 bg-gray-50/70">
      <div className="flex items-center gap-2.5">
        ...
        <span className="text-[10px] font-black text-[#1E3A5F] uppercase tracking-widest">{title}</span>
      </div>
      {right}
    </div>
  );
  ...
  return (
    <div className={`flex flex-col h-full border-l border-gray-200 bg-white transition-all duration-200 flex-shrink-0 ${collapsed ? "w-11" : "w-[320px]"}`}>
      ...
      <button onClick={onToggle}
        className="flex items-center justify-center gap-2 px-3 py-3.5 bg-[#1E3A5F] hover:bg-[#2E6DA4] transition-colors cursor-pointer flex-shrink-0 w-full">
        {collapsed ? (
              ...
              <div className="flex-shrink-0 px-3 py-2 border-b border-gray-200 bg-gray-50 space-y-2">
                <div className="flex gap-1 bg-white border border-gray-200 rounded-xs p-0.5">
                  {(["parsed", "raw"] as const).map(t => (
                    <button key={t} onClick={() => setTab(t)}
                      className={`flex-1 py-1 text-[9px] font-black uppercase tracking-wider rounded-xs transition-all cursor-pointer ${tab === t ? "bg-[#1E3A5F] text-white" : "text-gray-500 hover:text-[#1E3A5F]"}`}>
                      {t}
                    </button>
                  ))}
                </div>
                {remittance.download_url && (
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-1.5 text-[9px] font-black uppercase tracking-wider text-[#1E3A5F] hover:text-[#2E6DA4] border border-gray-200 hover:border-[#4A90E2] rounded-xs py-1.5 transition-colors disabled:opacity-50 cursor-pointer"
                  >
                            ...
                            {remittance.invoices!.map((inv: any, i: number) => {
                              ...
                              return (
                                <tr key={i} className={isThis ? "bg-blue-50" : "hover:bg-gray-50"}>
                                  <td className="px-2.5 py-2 font-mono font-bold text-[#1E3A5F]">
                                    {inv.invoice_number}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
        {topFields.map(([k, v]) => (
          <div key={k} className="bg-gray-50 border border-gray-200 rounded-xs px-3 py-2.5">
            ...
            <div className="text-[11px] font-mono font-bold text-[#1E3A5F] break-all">{v == null ? "—" : String(v)}</div>
          </div>
        ))}
      </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {refs.map((ref, i) => (
                  <tr key={i} className="hover:bg-blue-50/30">
                    <td className="px-3 py-2.5 font-mono font-bold text-[#1E3A5F]">{ref.ReferenceNumber}</td>
                    <td className="px-3 py-2.5 text-gray-500">{ref.ReceiptMatchBy}</td>
                    <td className="px-3 py-2.5 font-mono font-bold text-right text-[#1E3A5F]">{fmt(Number(ref.ReferenceAmount))}</td>
...
export default function RowDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] flex flex-col">
      ...
      {/* ── Top bar ──────────────────────────────────────────────────────────── */}
      <div className="bg-[#1E3A5F] px-5 py-0 flex items-stretch flex-shrink-0 shadow-md">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
                ...
                <div className="px-5 py-1">
                  ...
                  <div className="flex items-center justify-between gap-4 py-3 border-b border-gray-100">
                    ...
                    <span className="font-mono font-black text-[#1E3A5F]" style={{ fontSize: "22px", letterSpacing: "-0.02em" }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 divide-y sm:divide-y-0 sm:divide-x divide-gray-100">
                ...
                <div className="px-5 py-4">
                  ...
                  {ex.extracted_customer ? (
                    <>
                      <div className="text-[15px] font-black text-[#1E3A5F] mb-2.5 leading-snug">{ex.extracted_customer}</div>
                  ...
                  {(ex.all_invoice_numbers || []).length > 0 ? (
                    <div className="flex flex-wrap gap-1.5">
                      {ex.all_invoice_numbers.map(inv => (
                        <span key={inv}
                          className="font-mono text-[11px] font-black bg-[#1E3A5F]/5 text-[#1E3A5F] border border-[#1E3A5F]/15 px-2.5 py-1 rounded-xs">
                          {inv}
                        </span>
                      ))}
                    </div>
                      ...
                      {confirmed_invoices.map((inv, i) => {
                        ...
                        return (
                          <tr key={i} className="hover:bg-blue-50/20 transition-colors">
                            ...
                            <td className="px-4 py-3 font-mono font-bold text-right text-[#1E3A5F]">{fmt(inv.outstanding_amount)}</td>
                    ...
                    {confirmed_invoices.length > 1 && (
                      <tfoot className="border-t-2 border-gray-200">
                        <tr className="bg-gray-50">
                          ...
                          <td className="px-4 py-2.5 font-mono font-black text-right text-[#1E3A5F]">{fmt(sum_outstanding)}</td>
                          <td colSpan={2} />
                        ...
                        <tr className="bg-blue-50/50">
                          <td colSpan={3} className="px-4 py-2 text-[9px] font-black text-[#1E3A5F] uppercase tracking-wider">Bank credit amount</td>
                          <td colSpan={4} className="px-4 py-2 font-mono font-black text-right text-[#1E3A5F]">{fmt(credit_amount)} {bs.currency}</td>
                        </tr>
                      </tfoot>
                    )}
                  </table>
                ...
                {isCrossOU && (
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-gray-50 border border-gray-200 rounded-xs px-4 py-3">
                      ...
                      <div className="font-black text-[#1E3A5F] text-[14px] leading-snug">
                ...
                {sum_outstanding > 0 && credit_amount > 0 && (
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-gray-50 border border-gray-200 rounded-xs px-4 py-3">
                      ...
                      <div className="font-mono font-black text-[#1E3A5F] text-[18px] leading-none">{fmt(credit_amount)}</div>
                    ...
                    <div className="bg-gray-50 border border-gray-200 rounded-xs px-4 py-3">
                      ...
                      <div className="font-mono font-black text-[#1E3A5F] text-[18px] leading-none">{fmt(sum_outstanding)}</div>
