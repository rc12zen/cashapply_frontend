...
import MetricsKpiGrid from "../home/components/MetricsKpiGrid";
...
export default function OverviewPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-base font-bold text-[#222222]">Reconciliation Summary</h2>
        <p className="text-sm text-[#6B7688] mt-1">
      ...
      <div className="bg-white border border-gray-200 p-6 shadow-xs">
        <div className="mb-1">
          <h2 className="text-base font-bold text-[#222222]">Value Breakdown</h2>
          <p className="text-sm text-[#6B7688] mt-1">
            All credited amounts across categories, converted to USD.
          </p>
        </div>
        <MetricsKpiGrid
          emoji="💵" title="By Category" size="sm" columns="grid-cols-2 lg:grid-cols-3"
          formatValue={fmtUsd}
          items={[
            { icon: <DollarSign size={12} className="text-[#E4405D]" />, label: "Unidentified", value: ga.unidentified ?? 0, sub: "No matching customer or invoice", accent: "#E4405D" },
            { icon: <DollarSign size={12} className="text-[#F0A83C]" />, label: "Needs Remittance", value: ga.needs_remittance ?? 0, sub: "Awaiting remittance follow-up", accent: "#F0A83C" },
            { icon: <DollarSign size={12} className="text-[#1F9254]" />, label: "Ready for Oracle", value: ga.ready_for_oracle ?? 0, sub: "Exact match, ready to post", accent: "#1F9254" },
            { icon: <DollarSign size={12} className="text-[#C0392B]" />, label: "Conflict / Exception", value: ga.conflict_exception ?? 0, sub: "Mismatch needing SPOC review", accent: "#C0392B" },
            ...
            { icon: <DollarSign size={12} className="text-[#8A93A6]" />, label: "Rejected", value: ga.rejected ?? 0, sub: "Rejected by system or SPOC", accent: "#8A93A6" },
          ]}
        />
      </div>
